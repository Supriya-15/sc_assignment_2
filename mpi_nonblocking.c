...
int main(int argc, char * argv[]) {
    ...
    if (rank != 0) {
        ...
        MPI_Isend(&sq_points, 1, MPI_INT, 0, 0, MPI_COMM_WORLD, &request);
        MPI_Isend(&circle_points, 1, MPI_INT, 0, 0, MPI_COMM_WORLD, &request);
    }
    ...
    if (rank == 0) {
        for (int i = 0; i < size-1; i++) {
            ...
            MPI_Irecv(&inbuf_sq, 1, MPI_INT, MPI_ANY_SOURCE, 0, MPI_COMM_WORLD, &request);
            MPI_Wait(&request, MPI_STATUS_IGNORE);
            MPI_Irecv(&inbuf_circle, 1, MPI_INT, MPI_ANY_SOURCE, 0, MPI_COMM_WORLD, &request);
            MPI_Wait(&request, MPI_STATUS_IGNORE);
