...
int main(int argc, char* argv[])
{
    ...
    srand(SEED+myid);  
                         //Give rand() a seed value. Needs to be different on each node
 
 	if(nodenum<=1)
 	{
 	   printf("Pass minimum 2 number of processes to run");
 	   MPI_Finalize();
 	   return (0);
    ...
    else if (myid == 0)
    {
        for(i=0; i<nodenum; ++i)
        {
            ...
            MPI_Recv(&recvniter[i],nodenum,MPI_LONG,MPI_ANY_SOURCE,2,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
             ...
             printf("From rank %d, points in sq = %d, points in circle = %ld\n", i + 1, recieved[i], recvniter[i]);
  ...
  printf("\nTotal points received at rank 0: Circle = %d, Sq = %ld\n",finalcount, finalniter);
